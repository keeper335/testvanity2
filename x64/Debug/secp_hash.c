...
void secp256k1_sha256_write(secp256k1_sha256_t *hash, unsigned char *data, size_t len) {
...
void secp256k1_sha256_finalize(secp256k1_sha256_t *hash, unsigned char *out32) {
	...
	secp256k1_sha256_write(hash, (unsigned char*)sizedesc, 8);
	for (i = 0; i < 8; i++) {
		out[i] = BE32(hash->s[i]);
		hash->s[i] = 0;
	}
	memcpy(out32, (unsigned char*)out, 32);
}
